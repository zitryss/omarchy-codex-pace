...
- **50 tests passed** (`python3 -B -m unittest discover -s tests -v`, 10.5 seconds).
...
- The real panel rendered the three top metrics, two independently sized draining bars,
  frozen tracking baseline, exactly seven start-date calendar entries and reset count.
...
- Native pointer clicks opened, closed and reopened the icon's dropdown. An outside
  ...
  exercised live. IPC/keyboard summoning acquired focus correctly.
...
- Clearly labeled synthetic previews were inspected for ordinary carryover, debt/zero
  allowance, correction above 100%, stale data, expired reset and unavailable login.
...
- Disabling stopped the owned helper and preserved the ledger. Re-enabling restored
  the frozen plan. Reload and restart checks found no duplicate collectors.
...
close boundary estimates; stale/out-of-order readings; offline expiry; confirmed early
...
lock/reload deduplication; preserved server retry guidance; and simulated resume gaps.
...
and labeled previews. No reset credit was redeemed and no model turn was run by the collector.
