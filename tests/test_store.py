...
def reading(at=0,remaining='100',**kwargs):
    return dict(dict(account='test-account',bucket='test-bucket',start=0,end=WEEK,
       at=at,remaining=remaining,credits=None,provenance='test inferred',precision='reported integer',short=False),**kwargs)
...
class StoreTests(unittest.TestCase):
    ...
    def test_persist_frozen_plan(self):
        ...
        self.assertEqual((v['plan'],v['daily'],v['observed']),('14','93','1'))
    ...
    def test_repeated_readings_and_corrections_telescope(self):
        ...
        self.assertEqual(records[0]['observed'],1)
    ...
    def test_mid_bucket_first_run(self):
        ...
        self.assertEqual((v['plan'],v['available'],v['daily'],v['observed']),('16','12','75','—'))
        self.assertEqual(v['note'],'Since tracking began')
        self.assertEqual(self.store.records(self.store.get('reading'))[0]['plan'],None)

    def test_boundary_close_estimate(self):
        self.add(86380,'88')
        self.add(86420,'87')
        rec=self.store.records(self.store.get('reading'))[1]
        self.assertEqual(rec['opening'],88)
        self.assertIn('estimated',rec['quality'])
        self.assertEqual(rec['observed'],1)

    def test_boundary_gap_not_assigned(self):
        self.add(100,'98')
        self.add(86400*3+100,'80')
        records=self.store.records(self.store.get('reading'))
        self.assertIsNone(records[1]['observed'])
        self.assertIsNone(records[2]['observed'])
        self.assertIsNone(records[3]['observed'])
        self.assertEqual(records[3]['net'],0)
        self.assertEqual(records[3]['quality'],'Since tracking began')
    ...
    def test_stale_and_offline_reset(self):
        self.add(0)
        self.assertNotIn('Stale',project(self.store,119)['status'])
        self.assertIn('Stale',project(self.store,120)['status'])
    ...
    def test_early_reset_requires_confirmation(self):
        ...
        self.assertEqual(self.store.records(self.store.get('reading'))[0]['opening'],100)
    ...
    def test_accounts_and_buckets_never_merge(self):
        ...
        self.assertEqual(self.store.records(self.store.get('reading'))[0]['opening'],99)
